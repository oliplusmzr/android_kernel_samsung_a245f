...
static inline __always_inline bool updateRawData(int32_t event, uint64_t event_time,
...
static inline __always_inline void onSysEnterExecve(sys_enter_data_t *args) {
    ...
    bool res = updateRawData(event, event_time, syscall, cmdline_addr, false, false, data);
    if (!res) {
        return;
    }
...
static inline __always_inline void onSchedProcessExec(sched_process_exec_args_t *args) {
    ...
    bool res = updateRawData(event, event_time, syscall, cmdline_addr, true, false, data);
    if (!res) {
        return;
    }
...
static inline __always_inline void onTaskRename(task_rename_data_t *args) {
    ...
    bool res = updateRawData(event, event_time, syscall, cmdline_addr, true, true, data);
    if (!res) {
        return;
    }
