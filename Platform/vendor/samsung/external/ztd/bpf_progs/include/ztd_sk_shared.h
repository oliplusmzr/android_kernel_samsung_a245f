...
#define ENSURE_POSITIVE_STATE(oldstate, newstate) if (((oldstate) != BPF_TCP_CLOSE || (newstate) != BPF_TCP_LISTEN) \
                                            && ((oldstate) != BPF_TCP_SYN_SENT || (newstate) != BPF_TCP_ESTABLISHED) \
                                            && (newstate) != BPF_TCP_CLOSE) return;
