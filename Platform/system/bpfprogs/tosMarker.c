...
static __always_inline int set_tos_ipv4(struct __sk_buff *skb, uint16_t value) {
#ifndef __TEST__
...
static __always_inline int set_tos_ipv6(struct __sk_buff *skb, uint16_t value) {
#ifndef __TEST__
    void *data = (void *)(long)skb->data;
    const void *data_end = (void *)(long)skb->data_end;
...
static inline int classify_ack(struct __sk_buff *skb, bool is_ipv6) {
    ...
    uint16_t tcphdr_len = tcph->doff * 4;
...
DEFINE_OPTIONAL_BPF_PROG("schedcls/egress/set_tos_mobile", AID_ROOT, AID_SYSTEM, schedcls_egress_set_tos_mobile)
...
DEFINE_OPTIONAL_BPF_PROG("schedcls/egress/set_queue_mapping", AID_ROOT, AID_SYSTEM, schedcls_egress_set_queue_mapping)
(struct __sk_buff *skb) {
    if (skb->pkt_type != PACKET_HOST) {
        return TC_ACT_UNSPEC;
    }
...
DEFINE_OPTIONAL_BPF_PROG("schedcls/egress/classify_ack", AID_ROOT, AID_SYSTEM, schedcls_egress_classify_ack)
(struct __sk_buff *skb) {
    int ret = MISMATCH;
    ...
    if (skb->protocol == htons(ETH_P_IP)) {
        ret = classify_ack(skb, false);
    } else if (skb->protocol == htons(ETH_P_IPV6)) {
        ret = classify_ack(skb, true);
    }
...
DEFINE_OPTIONAL_BPF_PROG("schedcls/egress/classify_uid", AID_ROOT, AID_SYSTEM, schedcls_egress_classify_uid)
(struct __sk_buff *skb) {
    int ret = MISMATCH;
...
DEFINE_OPTIONAL_BPF_PROG("schedcls/egress/classify_queue_mapping", AID_ROOT, AID_SYSTEM, schedcls_egress_classify_queue_mapping)
(struct __sk_buff *skb) {
    if (skb->pkt_type != PACKET_HOST) {
        return MISMATCH;
    }
