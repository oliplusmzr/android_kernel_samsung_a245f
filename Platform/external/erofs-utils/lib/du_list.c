...
#ifdef WITH_ANDROID
static void dulist_write(struct erofs_inode *inode, erofs_blk_t nblocks)
{
	struct erofs_sb_info *sbi = inode->sbi;
	const char *fspath = erofs_fspath(inode->i_srcpath);
	erofs_off_t i_size = inode->i_size;

	fprintf(du_list_fp, "%12llu %12llu ", (unsigned long long)i_size,
			(unsigned long long)nblocks * erofs_blksiz(sbi));
...
void erofs_droid_dulist_write(struct erofs_inode *inode)
{
	...
	if (is_inode_layout_compression(inode))
		...
		nblks = BLK_ROUND_UP(inode->sbi, inode->i_size);

	dulist_write(inode, nblks);
}
#endif
