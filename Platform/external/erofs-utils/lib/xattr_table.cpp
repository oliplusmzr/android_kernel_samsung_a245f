...
public:
    ...
    bool setup_attrs(const string& filepath, char *kvbuf, unsigned int len[2])
    {
        ...
        len[0] = attrs->second.front().fullname.size();
        ...
        /*
        kvbuf must be in format: "<full_xattr_name>\0<data>"
        example: security.selinux\0u:object_r:unlabeled:s0"
        */
        snprintf(kvbuf, len[0] + 1, "%s", attrs->second.front().fullname.c_str());
        memcpy(kvbuf + len[0] + 1, attrs->second.front().data.data(), len[1]);
...
int xattr_table_setup(void *table, const char *filepath, char *kvbuf, unsigned int len[2])
{
    if (!table)
        return -1;
    XattrTable *t = reinterpret_cast<XattrTable *>(table);

    if (!t->setup_attrs(filepath, kvbuf, len)) {
        return -1;
    }
