...
struct f2fs_super_block {
	...
	__u8 reserved[174];		/* valid reserved region */
...
struct f2fs_extra_flag_blk {
	...
	__le32 ddp_stats[8];
	__le64 total_16k_overhead_blks;
	__u8 rsvd[4012];
} __attribute__((packed));
...
struct f2fs_orphan_block {
	__le32 ino[1];	/* F2FS_ORPHANS_PER_BLOCK inode numbers */
};
...
struct direct_node {
	__le32 addr[1];	/* array of DEF_ADDRS_PER_BLOCK data block address */
};

struct indirect_node {
	__le32 nid[1];	/* array of NIDS_PER_BLOCK data block address */
};
...
struct f2fs_nat_block {
	struct f2fs_nat_entry entries[1]; /* NAT_ENTRY_PER_BLOCK */
};
...
struct f2fs_sit_block {
	struct f2fs_sit_entry entries[1];
};
 ...
 */
struct nat_journal {
	struct nat_journal_entry entries[1];
};
 ...
 */
struct sit_journal {
	struct sit_journal_entry entries[1];
};
...
struct f2fs_dentry_block {
	...
	__u8 dentry_bitmap[1]; /* size is SIZE_OF_DENTRY_BITMAP, based on block size */
};
...
struct f2fs_configuration {
	...
	uint64_t total_16k_overhead_blks;
...
extern int __get_new_sb(struct f2fs_super_block *sb, uint64_t reserved_target);
