...
int dev_read(void *buf, __u64 offset, size_t len)
{
	...
	if (fd < 0)
		return fd;
#ifdef HAVE_PREAD
	res = pread(fd, buf, len, (off_t)offset);
	if (res < 0) {
		MSG(0, "\n pread fail.(%d) offset is %"PRIu64"\n", errno, offset);
		return -1;
	}
#else
...
#ifdef HAVE_PWRITE
	res = pwrite(fd, buf, len, (off_t)offset);
	if (res < 0) {
		MSG(0, "\n pwrite fail.(%d) offset is %"PRIu64"\n", errno, offset);
		return -1;
	}
#else
	if (lseek(fd, (off_t)offset, SEEK_SET) < 0) {
...
#define MAX_CHUNK_SIZE		(256 * 1024 * 1024 * 1024ULL)
