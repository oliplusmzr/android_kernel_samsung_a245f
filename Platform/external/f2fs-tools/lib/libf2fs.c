...
int get_device_info(int i)
{
	...
	if (c.wanted_total_sectors != -1) {
		...
		if (c.wanted_sector_size == -1) {
			...
			if (c.rsvd_meta_sectors) {
				c.rsvd_meta_sectors *= c.wanted_sector_size;
				c.rsvd_meta_sectors /= dev->sector_size;
			}
		}
	}
...
int __get_new_sb(struct f2fs_super_block *sb, uint64_t reserved_target)
{
	...
	if (reserved_target)
		set_sb(block_count, reserved_target >>
	...
	if (reserved_target)
		set_sb(segment_count, (reserved_target * c.sector_size -
