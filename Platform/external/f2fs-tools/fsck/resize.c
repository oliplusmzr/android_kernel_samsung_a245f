		...
		struct f2fs_super_block *new_sb, unsigned int offset)
{
	...
	for (type = 0; type < NR_CURSEG_NODE_TYPE; type++) {
		...
		ret = dev_write_block(curseg->sum_blk,
				GET_SUM_BLKADDR(sbi, segno), WRITE_LIFE_NONE);
		ASSERT(ret >= 0);
	}
	...
	for (seg = target_segs - 1; seg >= 0; seg--) {
		...
		for (blk = blocks_per_seg - 1; blk >= 0; blk--) {
			...
			if (blkaddr < expand_sum_blkaddr) {
				ret = dev_write_block((char *)ssa + blk * F2FS_BLKSIZE,
						blkaddr, WRITE_LIFE_NONE);
				ASSERT(ret >= 0);
			} else {
				ret = dev_write_block(zero_block, blkaddr, WRITE_LIFE_NONE);
				ASSERT(ret >= 0);
			}
		}
	}
...
static void erase_expanded_sit(struct f2fs_sb_info *sbi)
{
	...
	for (blkaddr = end_sit1_blkaddr; blkaddr < sit2_blkaddr; blkaddr++) {
		...
		ret = dev_write_block(zero_block,
				blkaddr + sit_blks, WRITE_LIFE_NONE);
		ASSERT(ret >= 0);
	}
...
static int backup_meta(struct f2fs_sb_info *sbi, u64 new_end_blkaddr)
{
	...
	for (i = 0; i < target_seg; i++) {
		...
		ASSERT(ret >= 0);
		ret = dev_write(buf, ((end_blkaddr + 2) * F2FS_BLKSIZE) +
				(i * seg_size), seg_size, WRITE_LIFE_NONE);
		ASSERT(ret >= 0);
	}
	ret = f2fs_fsync_device();
	ASSERT(ret >= 0);
	...
	for (i = 0; i < 2; i++) {
		...
		ret = dev_write_block(buf, end_blkaddr + i, WRITE_LIFE_NONE);
		ASSERT(ret >= 0);
	}
	ret = f2fs_fsync_device();
	ASSERT(ret >= 0);
	...
	for (i = 0; i < 2; i++) {
		...
		ASSERT(ret >= 0);
		memcpy(buf + F2FS_SUPER_OFFSET, sb, sizeof(*sb));
		ret = dev_write_block(buf, i, WRITE_LIFE_NONE);
		ASSERT(ret >= 0);
	}
	ret = f2fs_fsync_device();
	ASSERT(ret >= 0);
