...
#define OPT_RESERVE_META    (SEC_OPT_START + 2)
...
void resize_usage()
{
	...
	MSG(0, "  --reserve-metadata=#  # of total sectors to reserve metadata\n");
	exit(1);
}
...
void f2fs_parse_options(int argc, char *argv[])
{
	...
	if (!strcmp("fsck.f2fs", prog)) {
		...
		struct option long_opt[] = {
			...
			{"reserve-metadata", required_argument, 0, OPT_RESERVE_META},
			{0, 0, 0, 0}
		};
		...
		while ((option = getopt_long(argc, argv, option_string,
			...
			case OPT_FAIL_SAFE:
				...
				break;
			case OPT_RESERVE_META:
				c.rsvd_meta_sectors = strtoull(optarg, &endptr, 0);
				if (*endptr != '\0') {
					MSG(0, "\tError: invalid sectors to reserve metadata %s\n", optarg);
					resize_usage();
				}
				break;
			default:
		...
		if (c.safe_resize && c.rsvd_meta_sectors) {
			MSG(0, "\tError: safe_resize and reserve-metadata are mutually exclusive!\n");
			resize_usage();
		}
...
static int do_resize(struct f2fs_sb_info *sbi)
{
	...
	ret = f2fs_resize(sbi);
	stlog("Resize Info: %s %llu blocks, %u files, %u/%u nodes, %llu/%llu blocks\n",
			c.devices[0].path, (unsigned long long)get_sb(block_count),
			get_cp(valid_inode_count),
...
fsck_again:
	...
	if (ret != 0) {
		if (ret == 1) {
			...
			stlog("Info: No error was reported\n");
			...
			f2fs_do_umount(sbi);
			c.fix_on = 1;
			stlog("Error: Run fsck again w/ fix_on!\n");
