...
void update_free_segments(struct f2fs_sb_info *sbi)
{
	...
	MSG(1, "\r [ %c ] Free segments: 0x%x", progress[i % 5], SM_I(sbi)->free_segments);
	fflush(stdout);
	i++;
}
						...
						int want_type, bool new_sec)
{
	...
	if ((SM_I(sbi)->free_segments <= SM_I(sbi)->reserved_segments + 1) &&
					!(get_sb(feature) & F2FS_FEATURE_RO))
		not_enough = 1;
...
bypass_ssa:
	...
	/*
	 * Review ID - 31111850
	 * If it is an empty segment, it can be used as an LFO,
	 * so there is no need to unnecessarily change it to SSR
	 */
	se = get_seg_entry(sbi, curseg->segno);
	if (is_empty_segment(sbi, se, curseg->next_blkoff))
		curseg->alloc_type = LFS;
...
static int tune_sb_features(struct f2fs_sb_info *sbi)
{
	...
	if (get_sb(feature) & F2FS_FEATURE_COMPRESSION)
		def_mnt_opts = "flush_group=5666,compress_log_size=5";
	else
		def_mnt_opts = "flush_group=5666";
	...
	if (strlen(c.vol_label)) {
		...
		utf16_to_utf8(buffer, (const char *)sb->volume_name,
		...
		if (strncmp(buffer, c.vol_label, strlen(c.vol_label))) {
			...
			utf8_to_utf16((char *)sb->volume_name,
					(const char *)c.vol_label,
					MAX_VOLUME_NAME, strlen(c.vol_label));
			sb_changed = 1;
		}
	}

	if (get_sb(sec_reserved_blocks) > 524288ULL) {
		MSG(0, "Info: sec_reserved_blocks has invalid value: %llu\n",
				(unsigned long long)get_sb(sec_reserved_blocks));
		sb->sec_reserved_blocks = 0;
		sb_changed = 1;
	}
...
static void restore_backup_meta(struct f2fs_super_block *sb)
{
	...
	for (i = 0; i < target_segs; i++) {
		...
		ret = dev_write(buf, F2FS_BLKSIZE * (get_sb(segment0_blkaddr) +
					i * blocks_per_seg), seg_size, WRITE_LIFE_NONE);
		ASSERT(ret >= 0);
	}
...
static int disconnect_fsync_dnodes(struct f2fs_sb_info *sbi)
{
	struct curseg_info *curseg;
	struct f2fs_node *node_blk;
	block_t blkaddr;
	...
	snprintf((char *)F2FS_NODE_FOOTER(node_blk),
		sizeof(struct node_footer), "fsck:Disconn node chain");
...
out:
	...
	if (ret) {
		...
		if (ret == -ENOMEM)
			return -1;
		if (ret != -ELOOP) {
			ERR_MSG("disconnect fsync dnode\n");
			if (disconnect_fsync_dnodes(sbi)) {
