...
skip_dot_fix:
	...
	/* Estimate approx. overhead if 16K blocksize formatted */
	if (sbi->blocksize == 4096) {
		u32 allocated = *blk_cnt;

		/* inode_block */
		allocated -= 1;
		c.total_16k_overhead_blks += 3;

		/* xattr_block */
		if (node_blk->i.i_xattr_nid) {
			allocated -= 1;
			c.total_16k_overhead_blks += 3;
		}

		/* data_blocks, only count for remainder to make easier */
		c.total_16k_overhead_blks += allocated % 4 ? 4 - (allocated % 4) : 0;
	}
}
...
void fsck_update_extra_blk(struct f2fs_fsck *fsck)
{
	...
	extra_blk->total_16k_overhead_blks = cpu_to_le64(c.total_16k_overhead_blks);

	res = dev_write_block(extra_blk, extra_blk_no, WRITE_LIFE_NONE);
	if (res < 0)
...
static u64 get_max_image_blks(struct f2fs_sb_info *sbi)
{
	...
	if (max_image_segs < (F2FS_MIN_SEGMENTS + 1))
		return (F2FS_MIN_SEGMENTS + 1) * sbi->blocks_per_seg;
	else
	...
	if (force || (c.fix_on && f2fs_dev_is_writable())) {
		...
		if (force || c.bug_on || c.bug_nat_bits || c.quota_fixed) {
			...
			if (fsck_chk_need_fsck(sbi, true))
				...
				msg = "cp disabled";
			else if (c.invalid_sb & SB_ABNORMAL_STOP)
				msg = "abnormal stop";
			else if (c.invalid_sb & SB_FS_ERRORS)
				msg = "fs errors";
